...
export default function ChangePasswordPage() {
  ...
  // Password strength criteria (relaxed — min 6 chars for simplicity)
  const criteria = [
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-background/80 shadow-2xl backdrop-blur-xl border border-secondary relative z-10 animate-[fade-in_0.4s_ease-out]">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-foreground/50 text-sm">
            {user.mustChangePassword
              ? "يجب عليك تعيين كلمة مرور جديدة قبل الاستمرار"
        ...
        {success ? (
          <div className="text-center py-8">
            ...
            <a
              href={user.role === 'admin' || user.role === 'vice_president' ? "/admin" : "/member"}
              className="inline-block px-6 py-3 bg-primary text-primary-foreground rounded-xl font-bold hover:opacity-90 transition-opacity"
            >
              العودة للرئيسية
            </a>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 pl-11 rounded-xl bg-secondary/30 border border-secondary/50 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-foreground"
              ...
              <div className="flex flex-col gap-2">
                {criteria.map((c, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    {c.met
                      ...
                      : <X className="w-4 h-4 text-foreground/30 shrink-0" />}
                ...
                {confirmPassword && (
                  <div className="flex items-center gap-2 text-xs mt-1">
                    ...
                    <span className={newPassword === confirmPassword ? "text-green-600 font-medium" : "text-red-500"}>
            ...
            <div className="flex gap-3 pt-1">
              {!user.mustChangePassword && (
                <a
                  href={user.role === 'admin' || user.role === 'vice_president' ? "/admin" : "/member"}
                  className="flex-1 px-4 py-3 rounded-xl bg-secondary text-foreground font-bold text-center hover:bg-secondary/70 transition-colors"
                >
                  إلغاء
                </a>
              )}
              <button
                type="submit"
                disabled={!allMet || newPassword !== confirmPassword || isSubmitting}
                className={`flex-1 px-6 py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${
                  allMet && newPassword === confirmPassword && !isSubmitting
                    ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30 hover:opacity-90 cursor-pointer"
