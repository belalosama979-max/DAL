...
export function SuggestionBox() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addSuggestion({ title, content, author_id: user.id, theme: user.theme, category });
      toast("تم إرسال اقتراحك بنجاح! شكراً لمساهمتك.", "success");
      setTitle(""); setContent(""); setCategory("evaluation_team");
    } catch {
  ...
  return (
    <div className="bg-background border border-secondary rounded-3xl p-6 shadow-sm mb-8">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-full bg-yellow-500/10 text-yellow-500 flex items-center justify-center text-xl">💡</div>
        <div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="عنوان الفكرة أو الاقتراح"
        ...
        <textarea required value={content} onChange={e => setContent(e.target.value)} placeholder="تفاصيل الفكرة وطبيعتها..." rows={3}
        ...
        <button type="submit" disabled={isSubmitting || !title.trim() || !content.trim()}
          ...
          {isSubmitting ? "جاري الإرسال..." : <><span>إرسال الاقتراح</span><Send className="w-4 h-4" /></>}
        </button>
      </form>
    </div>
  );
